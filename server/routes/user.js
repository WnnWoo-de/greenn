...
import { userCache, cacheMiddleware } from '../utils/cacheService.js';
...
router.get('/leaderboard', 
  cacheMiddleware({ 
    ttl: 600000, // 10分钟缓存
    keyGenerator: (req) => `leaderboard:${req.query.type || 'points'}:${req.query.limit || 10}`,
  ...
  async (req, res) => {
    try {
      ...
      // Production mode: use database with optimization
      try {
        // 初始化模型
...
router.get('/:id', 
  ...
  cacheMiddleware({ 
    ttl: 300000, // 5分钟缓存
    keyGenerator: (req) => `user:${req.params.id}:${req.user?.id || 'public'}`,
    condition: (req) => req.method === 'GET'
  }),
  async (req, res) => {
    try {
      ...
      // 使用查询优化器获取用户信息
      const user = await queryOptimizer.optimizedQuery(UserModel, 'findByPk', {
        id: req.params.id,
        options: {
      ...
      if (ActivityModel && (preferences.privacy.showActivities || 
          (req.user && req.user.id === user.id))) {
        recentActivities = await queryOptimizer.optimizedQuery(ActivityModel, 'findAll', {
          options: { 
            where: { 
              userId: user.id, 
              isPublic: true 
          ...
          cacheKey: `user:${req.params.id}:activities`,
          cacheTTL: 180000 // 3分钟
        });
      }
